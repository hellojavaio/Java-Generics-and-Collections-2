...
此问题折磨框架中所有已排序集合的构造函数（`TreeSet`，`TreeMap`，`ConcurrentSkipListSet` 和 `ConcurrentSkipListMap`）。为了避免出现在你自己的类
...
图 `13-9` 的曲线箭头表示搜索元素 `55` 的进程。搜索从级别 `2` 的左上角的元素 `12` 开始，在该级别上步进到元素 `31`，然后发现下一个元素是 `61`，高于
